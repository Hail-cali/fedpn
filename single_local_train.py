...
def evaluate(model, data_loader, device, num_classes):
    model.eval()
    confmat = image_util.ConfusionMatrix(num_classes)
...
def main():
    ...
    if args.dataset == 'coco':
        ...
        # data_type = args.train_sets[0]
        # data_path = root + '/' + data_type + '/'
        # annFile = f'{root}/annotations/instances_{data_type}.json'

        dataset, num_classes = get_dataset(args.data_path, args.dataset, "train", get_transform(train=True), client=args.client_type)
        ...
        sample_model = "deeplabv3_mobilenet_v3_large_coco"
        # model = deeplabv3_mobilenet_v3_large(pretrained=args.pretrained)
        model = hail_mobilenet_v3_large(pretrained=args.pretrained)

        # model = torchvision.models.segmentation.__dict__[sample_model](num_classes=num_classes,
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            train_one_epoch(model, criterion, optimizer, data_loader, lr_scheduler, device, epoch, args.log_interval)
            confmat = evaluate(model, data_loader_test, device=device, num_classes=num_classes)
            print(confmat)
            checkpoint = {
            ...
            image_util.save_on_master(
                checkpoint,
                os.path.join(args.save_root, 'model_{}.pth'.format(epoch)))
            image_util.save_on_master(
                checkpoint,
                os.path.join(args.save_root, 'checkpoint.pth'))
